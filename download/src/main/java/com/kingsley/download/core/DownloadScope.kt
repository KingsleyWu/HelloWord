 ...
 * 代表一个下载任务
 * url将做为下载任务的唯一标识
 * 不要直接在外部直接创建此对象,那样就可能无法同一管理下载任务,请通过[DownloadUtil.request]获取此对象
 */
class DownloadScope(
    var url: String,
     ...
     * 才可以交由[DownloadUtil]进行下载任务的启动
     * 任务的开始可能并不是立即的,任务会受到[DownloadUtil]的管理
     */
    fun start() {
            ...
            downloadInfo?.let {
                ...
                when (downloadInfo.status) {
                    DownloadInfo.PAUSE, DownloadInfo.ERROR, DownloadInfo.NONE -> {
                        ...
                        DownloadUtil.launchScope(this@DownloadScope)
                    }
                }
            }
        }
    ...
    /**
     * 启动协程进行下载
     * 请不要尝试在外部调用此方法,那样会脱离[DownloadUtil]的管理
     */
    internal fun launch() = launch {
        try {
            download()
