...
internal fun Calendar.getWeeksOfYear(
    ...
    val weeksData = mutableListOf<MutableList<Long>>()
    for (i in 1..getMaxWeekOfYear(year)) {
        val daysData = getDaysOfWeek(year, i)
...
internal fun MutableList<Long>.contain(date: Long): Boolean {
    for (i in this) {
        if (StringUtils.conversionTime(i, "yyyyMMdd") ==
            StringUtils.conversionTime(date, "yyyyMMdd")
        ) {
            return true
        }
    }
...
internal fun Calendar.getDaysOfWeek(
    ...
    val weekData = mutableListOf<Long>()
    for (i in 0 until 7) {
