...
import com.kingsley.helloword.datepicker.controller.BaseDateTimeController
import com.kingsley.helloword.datepicker.controller.DateTimeController
import com.kingsley.helloword.datepicker.controller.DateTimeInterface
...
open class DateTimePicker : FrameLayout, DateTimeInterface {
    ...
    private var displayType = intArrayOf(DateTimeConfig.YEAR, DateTimeConfig.MONTH, DateTimeConfig.DAY, DateTimeConfig.HOUR, DateTimeConfig.MIN, DateTimeConfig.SECOND)
    ...
    constructor(context: Context, attrs: AttributeSet?) : super(context, attrs) {
        ...
        selectTextSize = px2dip(attributesArray.getDimensionPixelSize(R.styleable.DateTimePicker_dt_selectTextSize, 0).toFloat())
        normalTextSize = px2dip(attributesArray.getDimensionPixelSize(R.styleable.DateTimePicker_dt_normalTextSize, 0).toFloat())
    ...
    fun bindController(controller: BaseDateTimeController?) {
        ...
        if (this.controller == null)
            this.controller = DateTimeController()
                .bindPicker(DateTimeConfig.YEAR, mYearSpinner)
                .bindPicker(DateTimeConfig.MONTH, mMonthSpinner)
                .bindPicker(DateTimeConfig.DAY, mDaySpinner)
                .bindPicker(DateTimeConfig.HOUR, mHourSpinner)
                .bindPicker(DateTimeConfig.MIN, mMinuteSpinner)
                .bindPicker(DateTimeConfig.SECOND, mSecondSpinner)
                .bindGlobal(global)
                .build()
        else
            this.controller?.bindPicker(DateTimeConfig.YEAR, mYearSpinner)
                ?.bindPicker(DateTimeConfig.MONTH, mMonthSpinner)
                ?.bindPicker(DateTimeConfig.DAY, mDaySpinner)
                ?.bindPicker(DateTimeConfig.HOUR, mHourSpinner)
                ?.bindPicker(DateTimeConfig.MIN, mMinuteSpinner)
    ...
    fun setDisplayType(types: IntArray?) {
        ...
        if (!displayType.contains(DateTimeConfig.YEAR)) {
            mYearSpinner?.visibility = View.GONE
        }

        if (!displayType.contains(DateTimeConfig.MONTH)) {
            mMonthSpinner?.visibility = View.GONE
        }

        if (!displayType.contains(DateTimeConfig.DAY)) {
            mDaySpinner?.visibility = View.GONE
        }

        if (!displayType.contains(DateTimeConfig.HOUR)) {
            mHourSpinner?.visibility = View.GONE
        }

        if (!displayType.contains(DateTimeConfig.MIN)) {
            mMinuteSpinner?.visibility = View.GONE
        }

        if (!displayType.contains(DateTimeConfig.SECOND)) {
    ...
    fun setTextSize(@Dimension normal: Int, @Dimension select: Int) {
        if (normal == 0) return
        if (select == 0) return
        val textSize = dip2px(select.toFloat())
        val normalTextSize = dip2px(normal.toFloat())
    ...
    fun getPicker(displayType: Int): NumberPicker? {
        return when (displayType) {
            DateTimeConfig.YEAR -> mYearSpinner
            DateTimeConfig.MONTH -> mMonthSpinner
            DateTimeConfig.DAY -> mDaySpinner
            DateTimeConfig.HOUR -> mHourSpinner
            DateTimeConfig.MIN -> mMinuteSpinner
            DateTimeConfig.SECOND -> mSecondSpinner
            else -> null
        }
    }
