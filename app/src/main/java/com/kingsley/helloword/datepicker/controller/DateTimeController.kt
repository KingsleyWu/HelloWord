...
import com.kingsley.helloword.datepicker.DateTimeConfig
import com.kingsley.helloword.datepicker.getMaxDayInMonth
import com.kingsley.helloword.datepicker.isSameDay
import com.kingsley.helloword.datepicker.isSameHour
import com.kingsley.helloword.datepicker.isSameMinute
import com.kingsley.helloword.datepicker.isSameMonth
import com.kingsley.helloword.datepicker.isSameYear
...
open class DateTimeController : BaseDateTimeController() {
    ...
    override fun bindPicker(type: Int, picker: NumberPicker?): DateTimeController {
        when (type) {
            DateTimeConfig.YEAR -> mYearSpinner = picker
            DateTimeConfig.MONTH -> mMonthSpinner = picker
            DateTimeConfig.DAY -> mDaySpinner = picker
            DateTimeConfig.HOUR -> mHourSpinner = picker
            DateTimeConfig.MIN -> mMinuteSpinner = picker
            DateTimeConfig.SECOND -> mSecondSpinner = picker
        }
        return this
    }
    ...
    override fun build(): DateTimeController {
        ...
        mYearSpinner?.run {
            ...
            descendantFocusability = ViewGroup.FOCUS_BLOCK_DESCENDANTS //设置NumberPicker不可编辑
    ...
    override fun setWrapSelectorWheel(types: MutableList<Int>?, wrapSelector: Boolean) {
        ...
        wrapSelectorWheelTypes!!.apply {
            for (type in this) {
                when (type) {
                    DateTimeConfig.YEAR -> mYearSpinner?.run { wrapSelectorWheel = wrapSelector }
                    DateTimeConfig.MONTH -> mMonthSpinner?.run { wrapSelectorWheel = wrapSelector }
                    DateTimeConfig.DAY -> mDaySpinner?.run { wrapSelectorWheel = wrapSelector }
                    DateTimeConfig.HOUR -> mHourSpinner?.run { wrapSelectorWheel = wrapSelector }
                    DateTimeConfig.MIN -> mMinuteSpinner?.run { wrapSelectorWheel = wrapSelector }
                    DateTimeConfig.SECOND -> mSecondSpinner?.run { wrapSelectorWheel = wrapSelector }
                }
            }
        }
    }
