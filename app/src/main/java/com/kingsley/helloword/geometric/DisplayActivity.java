...
import com.kingsley.helloword.data.Shape;
import com.kingsley.helloword.data.ShapeDB;
import com.kingsley.helloword.data.ShapeHolder;
import com.kingsley.helloword.data.ShapeProcess;

public class DisplayActivity extends BaseActivity implements OnTouchListener, Callback {
    /**
     * SurfaceView对象
     */
    ...
    private void saveFile(String fileName) {
        try {
            ...
            wirte.flush();
            wirte.close();
            Toast.makeText(DisplayActivity.this, "保存成功：" + fileName, Toast.LENGTH_SHORT).show();
        } catch (Exception e) {
            Toast.makeText(DisplayActivity.this, "写入错误：" + e.getMessage(), Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void readFile(String fileName) {
        try {
            ObjectInput read = new ObjectInputStream(new FileInputStream(DisplayActivity.this.getFilesDir() + fileName));
            ShapeHolder readHolder = (ShapeHolder) read.readObject();
            read.close();
            shapeHolder = readHolder;
            process.setHolder(shapeHolder);
            drawDetail(0, 0);
            Toast.makeText(DisplayActivity.this, "读取成功:" + fileName, Toast.LENGTH_SHORT).show();
        } catch (Exception e) {
            Toast.makeText(DisplayActivity.this, "读取错误：" + e.getMessage(), Toast.LENGTH_SHORT).show();
        }
    }
        ...
        submit.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v1) {
                ...
                try {
                    ...
                    Toast.makeText(DisplayActivity.this, "请输入正确的数据：" + e.getMessage(), Toast.LENGTH_LONG).show();
                    return;
                }
                switch (v.getId()) {
                    case R.id.add_shape_cube:
                    ...
                    case R.id.add_shape_prism:
                        if (one > 20) {
                            Toast.makeText(DisplayActivity.this, "输入棱数过多  棱数范围必须小于等于20", Toast.LENGTH_LONG).show();
                            return;
                        }
